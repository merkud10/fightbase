...
function translateFightMethod(method: string | null | undefined, locale: "ru" | "en") {
  ...
  if (/^americana$/i.test(value)) return "Американа";
  if (/^triangle choke$/i.test(value)) return "Треугольник";
  if (/^d'arce choke$/i.test(value)) return "Дарс-чоук";
  if (/^anaconda choke$/i.test(value)) return "Анаконда";
  if (/^north.south choke$/i.test(value)) return "Удушение север-юг";
  if (/^ezekiel choke$/i.test(value)) return "Удушение Иезекииля";
  if (/^peruvian necktie$/i.test(value)) return "Перуанский галстук";
  if (/^von flue choke$/i.test(value)) return "Удушение Фон Флю";
  if (/^heel hook$/i.test(value)) return "Хил-хук";
  if (/^kneebar$/i.test(value)) return "Нибар";
  if (/^calf slicer$/i.test(value)) return "Кэлф-слайсер";
  if (/^neck crank$/i.test(value)) return "Шейный кранк";
  if (/^gogoplata$/i.test(value)) return "Гогоплата";
  if (/^twister$/i.test(value)) return "Твистер";
  if (/^bulldog choke$/i.test(value)) return "Бульдог-чоук";
  if (/^inverted triangle choke$/i.test(value)) return "Обратный треугольник";
  if (/^TKO \(Doctor.?s? stoppage\)$/i.test(value)) return "TKO (остановка врачом)";
  if (/^TKO \(punches\)$/i.test(value)) return "TKO (удары руками)";
  if (/^TKO \(elbows\)$/i.test(value)) return "TKO (удары локтями)";
  if (/^TKO \(body kick\)$/i.test(value)) return "TKO (удар ногой в корпус)";
  if (/^TKO \(head kick\)$/i.test(value)) return "TKO (удар ногой в голову)";
  if (/^TKO \(knee\)$/i.test(value)) return "TKO (удар коленом)";
  if (/^KO \(punch\)$/i.test(value)) return "KO (удар)";
  if (/^KO \(head kick\)$/i.test(value)) return "KO (удар ногой в голову)";
  if (/^No Contest$/i.test(value)) return "Несостоявшийся бой";
  if (/^Draw$/i.test(value)) return "Ничья";
